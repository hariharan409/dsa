...
class LinkedList {
    ...
    partitionList(x) {
        let dummy1 = new LinkedList();
        let dummy2 = new LinkedList();
        let temp = this.head;
        
        while(temp !== null) {
            temp.value < x ? 
            (!dummy1.head.value ? dummy1.head.value = temp.value : dummy1.push(temp.value)) : 
            (!dummy2.head.value ? dummy2.head.value = temp.value : dummy2.push(temp.value));
            temp = temp.next;
        }
        if(!dummy1.head.value) {
            return this.head = dummy2.head;
        }
        if(!dummy2.head.value) {
            return this.head = dummy1.head;
        }
        let current = dummy1.head;
        while(current.next) {
            current = current.next;
        }
        current.next = dummy2.head;
        this.head = dummy1.head; 
    }
